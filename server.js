...
app.get('/counts', async (request, response) => {
  ...
  const authHash = btoa(`${process.env.GNIP_USER}:${process.env.GNIP_PASS}`);
  setTimeout(async () => {
    const res = await get({
      url: url.href,
      options: {
